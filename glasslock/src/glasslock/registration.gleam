...
/// The service using WebAuthn to register or authenticate users (i.e. your
/// application).
...
/// Configuration for a registration request, built up via [`new`](#new) and the
/// setter functions, then handed to [`build`](#build) to produce browser options
...
/// A parsed registration response. Construct via
...
/// functions before calling [`build`](#build).
pub fn new(
...
/// [`build`](#build) and [`verify`](#verify) steps (signed cookie, database,
...
///
/// The first element is a `PublicKeyCredentialCreationOptionsJSON` value ready
...
/// [`build`](#build). Returns the verified credential on success.
pub fn verify(
...
fn verify_attestation(
  ...
  use attestation_object_cbor <- result.try(
