...
pub fn build_client_data_sets_ceremony_type_test() {
  let cases = [
    #(
  ...
  list.each(cases, fn(test_case) {
    let #(client_data, expected_type) = test_case
    let assert Ok(parsed) = internal.parse_client_data(client_data)
    assert parsed.type_ == expected_type
  })
}
