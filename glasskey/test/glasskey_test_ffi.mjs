...
function binaryExtensionResults() {
  const backing = new Uint8Array([9, 1, 2, 3, 9]);
  return {
    credProps: { rk: true },
    prf: {
      results: {
        first: new Uint8Array([251, 255]).buffer,
        second: new Uint8Array(backing.buffer, 1, 3),
      },
    },
  };
}
...
export function lastCreateSnapshot() {
  ...
  return Result$Ok(
    CreateSnapshot$CreateSnapshot(
      ...
      pk.authenticatorSelection.residentKey,
      pk.authenticatorSelection.userVerification,
      pk.attestation,
