...
pub fn decode_cose_key_map_test() {
  ...
  let cose_key_bytes = <<
    ...
    0x21, 0x58, 0x20, x:bits,
    // Key -3 (y): 22 -> Value byte string 32 bytes: 58 20 <32 bytes>
    0x22, 0x58, 0x20, y:bits,
  >>
  let assert Ok(cbor.Map(entries)) = cbor.decode_all(cose_key_bytes)
  ...
  assert list.key_find(entries, cbor.Int(-2)) == Ok(cbor.Bytes(x))
  assert list.key_find(entries, cbor.Int(-3)) == Ok(cbor.Bytes(y))
}
...
pub fn decode_rejects_unsupported_major_type_test() {
  assert cbor.decode(<<0x80>>) == Error("Unsupported CBOR major type: 4")
}

pub fn decode_rejects_value_exceeding_53_bit_range_test() {
  assert cbor.decode(<<0x1B, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00>>)
    == Error("CBOR value exceeds 53-bit range")
}

pub fn decode_rejects_invalid_utf8_text_string_test() {
  assert cbor.decode(<<0x61, 0xFF>>)
...
pub fn decode_rejects_excessive_nesting_test() {
  assert cbor.decode_all(nested_single_entry_map(17))
    == Error("CBOR nesting depth exceeds 16")
}

pub fn decode_rejects_oversized_map_test() {
  let map_with_257_entries = <<0xB9, 0x01, 0x01, 0:size(257 * 16)>>
  assert cbor.decode_all(map_with_257_entries)
    == Error("CBOR map exceeds 256 entries")
}

fn nested_single_entry_map(depth: Int) -> BitArray {
  case depth {
    0 -> <<0>>
    _ -> <<0xA1, 0, nested_single_entry_map(depth - 1):bits>>
  }
}
