...
pub type Error {
  ...
  InvalidSignature
  /// The credential ID is not in the allowed credentials list or does not match
...
/// Configuration for an authentication request, built up via [`new`](#new) and
/// the setter functions, then handed to [`build`](#build) to produce browser
/// options and a challenge verifier.
...
/// A parsed authentication response. Construct via
/// [`response_decoder`](#response_decoder) (when the response arrives nested in
/// a larger JSON envelope) or [`parse_response_json`](#parse_response_json)
/// (when you have a raw response string). Pass to
...
/// Add an additional accepted origin. The origin passed to [`new`](#new) is
/// always included. The signed `clientDataJSON.origin` returned by the
/// authenticator must match one of them exactly, so pass a serialized origin
...
/// Add a credential to the `allow_credentials` list. Pass the stored
...
///
/// The first element is a `PublicKeyCredentialRequestOptionsJSON` value ready
/// to serialize or embed inside a response envelope. The second is the
/// verifier to pass to [`verify`](#verify).
...
/// nested in a larger JSON envelope.
...
/// Parse a raw response JSON string into a [`Response`](#Response).
...
/// Extract the credential id and optional user handle for lookup (discoverable
/// flow).
///
...
/// arrives as a raw JSON string: parses with [`parse_response_json`](#parse_response_json) then
/// verifies.
pub fn verify_json(
