...
- Durable storage. `backend/credentials.gleam` is a `storail`-backed file store intended for the demo. Replace it with a real database, including unique constraints on credential ID and user ID, plus the appropriate indexes for credential lookup (by credential ID for the discoverable flow, and by user ID for building `allow_credentials` from a known username).
